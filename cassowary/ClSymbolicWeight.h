...
class ClSymbolicWeight {
  ...
  public:
    ...
    ClSymbolicWeight(const std::vector<Number> &weights);
    ...
    ClSymbolicWeight operator*(const Number &n) const { return Times(n); }

    ClSymbolicWeight operator/(const Number &n) const { return DivideBy(n); }

    // FIXGJB: can we express this statically?
    ClSymbolicWeight operator*(ClSymbolicWeight &w) const {
        throw ExCLInternalError(
            "Multiplication of symbolic weights encountered");
        return w;
    }
    ClSymbolicWeight &operator*=(ClSymbolicWeight &w) {
        throw ExCLInternalError(
            "Multiplicative assignment of symbolic weights encountered");
        return w;
    }
    ...
    ClSymbolicWeight &operator*=(const Number &n) { return MultiplyMe(n); }
    ...
    friend bool operator==(const ClSymbolicWeight &cl1,
                           const ClSymbolicWeight &cl2) {
    ...
    double AsDouble() const {
        std::vector<Number>::const_reverse_iterator i = _values.rbegin();
        Number sum = 0;
        Number factor = 1;
    ...
    std::ostream &PrintOn(std::ostream &xo) const {
        std::vector<Number>::const_iterator i = _values.begin();
        if (i == _values.end())
            return xo;
    ...
    friend std::ostream &operator<<(std::ostream &xos,
                                    const ClSymbolicWeight &clsw) {
        clsw.PrintOn(xos);
        return xos;
    }
#endif

    int CLevels() const { return _values.size(); }

    friend bool ClApprox(const ClSymbolicWeight &cl, Number n);
    friend bool ClApprox(const ClSymbolicWeight &cl1,
                         const ClSymbolicWeight &cl2);

  private:
    std::vector<Number> _values;

    void push_back(Number d) { _values.push_back(d); }
};

inline bool ClApprox(const ClSymbolicWeight &cl, Number n) {
    std::vector<Number>::const_iterator it = cl._values.begin();
    if (!ClApprox(*it, n))
        return false;
...
inline bool ClApprox(const ClSymbolicWeight &cl1, const ClSymbolicWeight &cl2) {
    std::vector<Number>::const_iterator it1 = cl1._values.begin();
    std::vector<Number>::const_iterator it2 = cl2._values.begin();

    for (; it1 != cl1._values.end() && it2 != cl2._values.end(); ++it1, ++it2) {
        if (!ClApprox(*it1, *it2))
            return false;
    }
