...
inline enum ClCnRelation ReverseInequality(enum ClCnRelation c) {
    if (c != cnNEQ)
...
class ClConstraint {
#endif
  public:
    ClConstraint(const ClStrength &strength = ClsRequired(),
                 double weight = 1.0)
        : _strength(strength), _readOnlyVars(), _weight(weight), _pv(0),
          _times_added(0) {
        CtrTracer(__FUNCTION__, this);
    }

    virtual ~ClConstraint() { DtrTracer(__FUNCTION__, this); }
    ...
    virtual bool IsEditConstraint() const { return false; }
    ...
    virtual bool IsStrictInequality() const { return false; }

    virtual bool IsRequired() const { return _strength.IsRequired(); }

    virtual bool isStayConstraint() const { return false; }

    virtual const ClStrength &strength() const { return _strength; }
    ...
    const ClVarSet &ReadOnlyVars() const { return _readOnlyVars; }

    ClConstraint &AddROVars(const ClVarSet &setClv) {
        for (ClVarSet::const_iterator it = setClv.begin(); it != setClv.end();
             ++it) {
  ...
  private:
    ...
    void addedTo(const ClSimplexSolver &) { ++_times_added; }

    void removedFrom(const ClSimplexSolver &) { --_times_added; }

    void setStrength(const ClStrength &strength) { _strength = strength; }
