...
class ClLinearInequality : public ClLinearConstraint {
  private:
    typedef ClLinearConstraint super;
  ...
  public:
    ...
    ClLinearInequality(const ClVariable clv, ClCnRelation op,
        ...
        if (op == cnGEQ || op == cnGT) {
            ...
            throw ExCLEditMisuse(
                "Cannot use that operator for ClLinearInequality objects");
        }
        if (op == cnLT || op == cnGT) {
    ...
    ClLinearInequality(const ClLinearExpression &cle, ClCnRelation op,
    ...
    ClLinearInequality(const ClLinearExpression &cle1, ClCnRelation op,
    ...
    ClLinearInequality(const ClVariable clv1, ClCnRelation op,
    ...
    // that it is not.
    virtual bool IsInequality() const { return true; }
