...
class ClTableau {
  ...
  public:
    ...
    void NoteRemovedVariable(ClVariable v, ClVariable subject) {
        ...
        cerr << "v = " << v << " and Columns[v].size() = " << column.size()
             << endl;
#endif
        if (column.size() == 0) {
            _columns.erase(v);
  ...
  protected:
    ...
    ClTableauColumnsMap Columns() { return _columns; }

    ClTableauRowsMap Rows() { return _rows; }
    ...
    bool FIsBasicVar(ClVariable v) { return RowExpression(v) != NULL; }
    ...
    // this was added to the C++ version to reduce time in
