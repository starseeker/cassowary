...
// and replace ASAP
template <class T> class cl_auto_ptr {
  public:
    explicit cl_auto_ptr(T *p = 0) : pointee(p) {}

    template <class U>
    cl_auto_ptr(cl_auto_ptr<U> &rhs)
        : pointee(rhs.release()) {}

    ~cl_auto_ptr() { delete pointee; }

    template <class U> cl_auto_ptr<T> &operator=(cl_auto_ptr<U> &rhs) {
        if (this != &rhs)
            reset(rhs.release());
        return *this;
    }
...
template <class T> void ReinitializeAutoPtr(cl_auto_ptr<T> &apref, T *pt) {
    apref.reset(pt);
}
