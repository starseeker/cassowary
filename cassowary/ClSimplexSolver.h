// $Id: ClSimplexSolver.h,v 1.80 2006/12/29 21:57:13 gjbadros Exp $
//
...
class ClSimplexSolver : public ClSolver, public ClTableau {
  ...
  public:
    // Constructor
    ClSimplexSolver()
        : ClSolver(), _objective(ClVariable(new ClObjectiveVariable("Z"))),
          _slackCounter(0), _artificialCounter(0),
#ifdef CL_FIND_LEAK
          _cArtificialVarsDeleted(0),
#endif
          _dummyCounter(0), _epsilon(1e-8),
          _fResetStayConstantsAutomatically(true), _fNeedsSolving(false),
          _fExplainFailure(false), _pfnResolveCallback(NULL),
    ...
    ClSimplexSolver &AddLowerBound(ClVariable v, Number lower) {
        ClLinearInequality *pcn =
            new ClLinearInequality(ClLinearExpression(v.Value() - lower));
        return AddConstraint(pcn);
    }
    ClSimplexSolver &AddUpperBound(ClVariable v, Number upper) {
        ClLinearInequality *pcn =
            new ClLinearInequality(ClLinearExpression(upper - v.Value()));
        return AddConstraint(pcn);
    }
    ...
    ClSimplexSolver &AddConstraint(ClConstraint *const pcn) throw(
    ...
    ClSimplexSolver &AddConstraint(ClConstraint &cn) throw(
        ExCLTooDifficultSpecial, ExCLStrictInequalityNotAllowed,
        ExCLReadOnlyNotAllowed, ExCLEditMisuse, ExCLRequiredFailure,
        ExCLRequiredFailureWithExplanation, ExCLInternalError) {
    ...
    ClSimplexSolver &
    AddEditVar(const ClVariable v, const ClStrength &strength = ClsStrong(),
               double weight = 1.0) throw(ExCLTooDifficultSpecial,
                                          ExCLStrictInequalityNotAllowed,
                                          ExCLReadOnlyNotAllowed,
                                          ExCLEditMisuse, ExCLRequiredFailure) {
        ClEditConstraint *pedit = new ClEditConstraint(v, strength, weight);
        return AddConstraint(pedit);
    }

    ClSimplexSolver &RemoveEditVar(ClVariable v) throw(ExCLEditMisuse) {
    ...
    ClSimplexSolver &BeginEdit() throw(ExCLEditMisuse) {
    ...
    // for now, it just removes edit variables added from before the last
    // BeginEdit
    ClSimplexSolver &EndEdit() throw(ExCLEditMisuse) {
        if (_editInfoList.size() == 0)
    ...
    ClSimplexSolver &AddPointStay(const ClVariable vx, const ClVariable vy,
                                  const ClStrength &strength = ClsWeak(),
                                  double weight = 1.0) {
        AddStay(vx, strength, weight);
        AddStay(vy, strength, weight);
        return *this;
    }
    ...
    ClSimplexSolver &
    RemoveConstraint(ClConstraint *const pcn) throw(ExCLConstraintNotFound) {
        RemoveConstraintInternal(pcn);
        pcn->removedFrom(*this);
        return *this;
    }
    ...
    // Deprecated! --02/19/99 gjb
    ClSimplexSolver &
    RemoveConstraint(ClConstraint &cn) throw(ExCLConstraintNotFound) {
    ...
    ClSimplexSolver &SuggestValue(ClVariable v, Number x) throw(ExCLEditMisuse);
    ...
    bool FIsExplaining() const { return _fExplainFailure; }
    ...
    const ClConstraintToVarMap &ConstraintMap() const { return _markerVars; }

    const ClVarToConstraintMap &MarkerMap() const { return _constraintsMarked; }
    ...
    // variable)
    void UpdateExternalVariables() { SetExternalVariables(); }

    // A. Beurive' Tue Jul  6 17:05:39 CEST 1999
    void ChangeStrengthAndWeight(ClConstraint *pcn, const ClStrength &strength,
                                 double weight);
    ...
    ClSimplexSolver &SetAutoResetStayConstants(bool f) {
        _fResetStayConstantsAutomatically = f;
        if (f)
            ResetStayConstants();
        return *this;
    }
    ...
    class ClEditInfo {
      ...
      public:
        ...
        ClEditInfo(ClVariable clv, ClEditConstraint *pconstraint,
                   ClVariable eplus, ClVariable eminus, Number prevEditConstant)
            : _clv(clv), _pconstraint(pconstraint), _clvEditPlus(eplus),
              _clvEditMinus(eminus), _prevEditConstant(prevEditConstant) {}
    ...
    void BuildExplanation(ExCLRequiredFailureWithExplanation &e, ClVariable av,
    ...
    // false if not.
    bool TryAddingDirectly(ClLinearExpression &pexpr) throw(
        ExCLRequiredFailureWithExplanation, ExCLInternalError,
        ExCLRequiredFailure);
    ...
    ClVariable ChooseSubject(ClLinearExpression &pexpr) throw(
        ExCLRequiredFailureWithExplanation, ExCLInternalError,
        ExCLRequiredFailure);
    ...
    // Do a Pivot.  Move entryVar into the basis (i.e. make it a basic
    // variable),
    ...
    ClSimplexSolver &RemoveConstraintInternal(
