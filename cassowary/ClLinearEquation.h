...
class ClLinearEquation : public ClLinearConstraint {
  ...
  public:
    ...
    ClLinearEquation(ClVariable clv, const ClLinearExpression &cle,
                     const ClStrength &strength = ClsRequired(),
                     double weight = 1.0)
        : ClLinearConstraint(cle, strength, weight) {
    ...
    ClLinearEquation(const ClLinearExpression &cle, ClVariable clv,
                     const ClStrength &strength = ClsRequired(),
                     double weight = 1.0)
        : ClLinearConstraint(cle, strength, weight) {
        _expression.AddVariable(clv, -1.0);
    ...
    virtual bool FIsSatisfied() const { return (_expression.Evaluate() == 0); }
};
