...
class ClFloatVariable : public ClAbstractVariable {
  public:
    ...
    ClFloatVariable(Number Value = 0.0)
        : ClAbstractVariable(""), _value(Value) {}

    ClFloatVariable(long number, char *prefix, Number Value = 0.0)
        : ClAbstractVariable(number, prefix), _value(Value) {}

    virtual bool IsFloatVariable() const { return true; }
    ...
    virtual bool IsPivotable() const { return false; }
    ...
    virtual bool IsRestricted() const { return false; }
    ...
    virtual void ChangeValue(Number Value) { _value = Value; }
