...
class ClVariable {
  ...
  public:
    ...
    ClVariable(Number Value = 0.0) : pclv(new ClFloatVariable(Value)) {}
    ...
    ClVariable(ClFDVariable *pcfv) : pclv(pcfv) {
        if (pmapStrPclv) {
            (*pmapStrPclv)[pcfv->Name()] = *this;
        }
    }
    ...
    bool IsFloatVariable() const {
        assert(pclv);
        return pclv->IsFloatVariable();
    }
    bool IsFDVariable() const {
        assert(pclv);
        return pclv->IsFDVariable();
    }
    bool IsDummy() const {
        assert(pclv);
        return pclv->IsDummy();
    ...
    bool IsExternal() const {
        assert(pclv);
        return pclv->IsExternal();
    }
    bool IsPivotable() const {
        assert(pclv);
        return pclv->IsPivotable();
    }
