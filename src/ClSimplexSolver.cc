// $Id: ClSimplexSolver.cc,v 1.119 2005/10/20 04:25:21 gjb Exp $
//
...
//					  ExCLRequiredFailureWithExplanation, ExCLInternalError)
{
    ...
    if (!pcn->FIsOkayForSimplexSolver()) {
        throw ExCLTooDifficultSpecial(
            "SimplexSolver cannot handle this constraint object");
    }
    ...
    if (pcn->IsEditConstraint()) {
        ...
        if (!v.IsExternal() || (!FIsBasicVar(v) && !ColumnsHasKey(v))) {
            ...
            throw ExCLEditMisuse(
                "(ExCLEditMisuse) Edit constraint on variable not in tableau.");
        }
    ...
    ClLinearExpression *pexpr =
        NewExpression(pcn, /* output to: */
                      clvEplus, clvEminus, prevEConstant);
...
#ifdef CL_TRACE
        cerr << "could not Add directly -- caught ExCLRequiredFailure error"
             << endl;
#endif
    ...
    if (!fAddedOkDirectly) { // could not Add directly
    ...
    if (pcn->IsEditConstraint()) {
        ...
        _editInfoList.push_back(
            new ClEditInfo(clv, pcnEdit, clvEplus, clvEminus, prevEConstant));
    }
...
ClSimplexSolver &ClSimplexSolver::AddPointStay(const ClPoint &clp,
                                               const ClStrength &strength,
                                               double weight) {
...
ClSimplexSolver &ClSimplexSolver::RemoveEditVarsTo(int n) {
    ...
    for (ClEditInfoList::const_iterator it = _editInfoList.begin();
         (it != _editInfoList.end() &&
          _editInfoList.size() != static_cast<unsigned int>(n));
         ++it, ++i) {
        const ClEditInfo *pcei = (*it);
        assert(pcei);
#ifdef DEBUG_NESTED_EDITS
        cerr << __FUNCTION__ << "Checking " << pcei->_clv << ", index = " << i
             << endl;
#endif
        if (i >= n)
            qclv.push(pcei->_clv);
 	...
 	// handle exceptions: ExCLConstraintNotFound
{
    ...
    if (fFoundErrorVar) {
        ...
        for (; it != eVars.end(); ++it) {
            ...
            if (pexpr == NULL) {
                pzRow->AddVariable(
                    *it, -pcn->weight() *
                             pcn->strength().symbolicWeight().AsDouble(),
                    _objective, *this);
    ...
    ClConstraintToVarMap::iterator it_marker = _markerVars.find(pcn);
...
#endif
    if (!FIsBasicVar(marker)) { // not in the basis, so need to do some work
...
#ifdef CL_TRACE
                cerr << "Marker " << marker << "'s coefficient in " << *pexpr
                     << " is " << coeff << endl;
#endif
        ...
        if (!fExitVarSet) { // exitVar is still nil
    ...
    if (pcn->isStayConstraint()) {
        ...
        if (fFoundErrorVar) {
            ClVarSet &eVars = (*it_eVars).second;
            _stayPlusErrorVars.erase(remove_if(_stayPlusErrorVars.begin(),
                                               _stayPlusErrorVars.end(),
                                               VarInVarSet(eVars)),
                                     _stayPlusErrorVars.end());
            _stayMinusErrorVars.erase(remove_if(_stayMinusErrorVars.begin(),
                                                _stayMinusErrorVars.end(),
                                                VarInVarSet(eVars)),
                                      _stayMinusErrorVars.end());
        }
    } else if (pcn->IsEditConstraint()) {
        const ClEditConstraint *pcnEdit =
            dynamic_cast<const ClEditConstraint *>(pcn);
        const ClVariable clv = pcnEdit->variable();
        ClEditInfo *pcei = PEditInfoFromClv(clv);
        assert(pcei);
        ClVariable clvEditMinus = pcei->_clvEditMinus;
        RemoveColumn(
            clvEditMinus); // clvEditPlus is a marker var and gets removed later
        delete pcei;
...
void ClSimplexSolver::Resolve() { // CODE DUPLICATED ABOVE
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
#endif
    DualOptimize();
...
bool ClSimplexSolver::AddWithArtificialVariable(
    ClLinearExpression &expr, ExCLRequiredFailureWithExplanation &e) {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "(" << expr << ")" << endl;
#endif
...
#ifdef CL_TRACE
    cerr << "pazTableauRow->Constant() == " << pazTableauRow->Constant()
         << endl;
#endif
...
//					  ExCLRequiredFailure)
{
    ...
    for (; it != terms.end(); ++it) {
        ...
        } else { // we haven't found an restricted variable yet
            if (v.IsRestricted()) {
...
void ClSimplexSolver::DeltaEditConstant(Number delta, ClVariable plusErrorVar,
                                        ClVariable minusErrorVar) {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "(" << delta << ", " << plusErrorVar << ", " << minusErrorVar << ")"
         << endl;
#endif
...
void ClSimplexSolver::DualOptimize() {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "()" << endl;
#endif
            ...
            if (pexpr->Constant() < 0.0) {
                ...
                for (; it != terms.end(); ++it) {
                    ...
                    if (c > 0.0 && v.IsPivotable()) {
                        ...
                        r = zc /
                            c; // FIXGJB r:= zc/c or Zero, as ClSymbolicWeight-s
                        if (r < ratio) {
                            entryVar = v;
                            ratio = r;
                        }
...
ClLinearExpression *ClSimplexSolver::NewExpression(const ClConstraint *pcn,
                                                   /* output to */
                                                   ClVariable &clvEplus,
    ...
    if (pcn->IsInequality()) {
        ...
        if (!pcn->IsRequired()) {
            ...
            ClLinearExpression *pzRow = RowExpression(_objective);
            // FIXGJB:
            // pzRow->AddVariable(eminus,pcn->strength().symbolicWeight() *
            // pcn->weight());
            ClSymbolicWeight sw =
                pcn->strength().symbolicWeight().Times(pcn->weight());
            pzRow->setVariable(*peminus, sw.AsDouble());
            _errorVars[pcn].insert(peminus.get());
            NoteAddedVariable(*peminus, _objective);
        }
    } else { // cn is an equality
        if (pcn->IsRequired()) {
            ...
            ClLinearExpression *pzRow = RowExpression(_objective);
            // FIXGJB: pzRow->AddVariable(eplus,pcn->strength().symbolicWeight()
            // * pcn->weight());
            ClSymbolicWeight sw =
                pcn->strength().symbolicWeight().Times(pcn->weight());
    ...
    if (pexpr->Constant() < 0) {
#ifdef CL_TRACE
        cerr << "NewExpression's Constant is " << pexpr->Constant()
             << ", < 0, so flipping" << endl;
#endif
        pexpr->MultiplyMe(-1);
    }
    ...
    // (it releases the cl_auto_ptr from the responsibility of deleting the
    // object)
    pslackVar.release();
    pdummyVar.release();
    peminus.release();
...
// be feasible.)
void ClSimplexSolver::Optimize(ClVariable zVar) {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "(" << zVar << ")\n" << *this << endl;
#endif
    ...
    while (true) {
        ...
        for (; it != terms.end(); ++it) {
            ClVariable v = (*it).first;
            Number c = (*it).second;
            //      if (v.IsPivotable() && c < 0.0 && (entryVar == clvNil ||
            //      v.get_pclv() < entryVar.get_pclv()))
        ...
        cerr << "entryVar == " << entryVar << ", "
             << "objectiveCoeff == " << objectiveCoeff << endl;
#endif
...
void ClSimplexSolver::Pivot(ClVariable entryVar, ClVariable exitVar) {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
...
void ClSimplexSolver::ResetStayConstants() {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "()" << endl;
#endif
    ClVarVector::const_iterator itStayPlusErrorVars =
        _stayPlusErrorVars.begin();
    ClVarVector::const_iterator itStayMinusErrorVars =
        _stayMinusErrorVars.begin();
...
// them."
void ClSimplexSolver::SetExternalVariables() {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "()\n" << *this << endl;
#endif
...
void ClSimplexSolver::ChangeStrengthAndWeight(ClConstraint *pcn,
                                              const ClStrength &strength,
                                              double weight) {
    ClConstraintToVarSetMap::iterator it_eVars = _errorVars.find(pcn);
    // Only for constraints that already have error variables (i.e. non-required
    // constraints)
    ...
    Number old_coeff =
        pcn->weight() * pcn->strength().symbolicWeight().AsDouble();
...
#ifdef CL_TRACE
        cerr << "Changing strength and/or weight for constraint: " << endl
             << *pcn << endl;
...
void ClSimplexSolver::ChangeStrength(ClConstraint *pcn,
                                     const ClStrength &strength) {
    ChangeStrengthAndWeight(pcn, strength, pcn->weight());
}

// A. Beurive' Tue Jul  6 17:03:42 CEST 1999
void ClSimplexSolver::ChangeWeight(ClConstraint *pcn, double weight) {
