...
void ClTableau::addRow(ClVariable var, const ClLinearExpression &expr) {
#ifdef CL_TRACE
    ...
    // for each variable in expr, Add var to the set of rows which have that
    // variable
    // in their Expression
...
// Remove the parametric variable var, updating the appropriate column and row
// entries.
// (Renamed from Smalltalk implementation's `removeParametricVar')
ClVariable ClTableau::RemoveColumn(ClVariable var) {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "(" << var << ")" << endl;
#endif
...
ClLinearExpression *ClTableau::RemoveRow(ClVariable var) {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "(" << var << ")" << endl;
#endif
...
void ClTableau::SubstituteOut(ClVariable oldVar,
