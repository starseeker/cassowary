...
ClFDSolver &ClFDSolver::AddConstraint(ClConstraint *const pcn) {
...
ClFDSolver &ClFDSolver::RemoveConstraint(ClConstraint *const pcn) {
...
ClFDSolver &ClFDSolver::AddConstraintInternal(ClConstraint *const pcn) {
#ifdef CL_TRACE
    ...
    if (!pcnfd) {
        throw ExCLTooDifficultSpecial(
            "Can only add ClFDBinaryOneWayConstraint-s to ClFDSolvers");
    }
    ...
    if (!(ro.IsNil() || ro.IsFDVariable())) {
        throw ExCLTooDifficultSpecial(
            "RO variable must be an FDVariable or clvNil");
    }
...
ClFDSolver &ClFDSolver::RemoveConstraintInternal(ClConstraint *const pcn) {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "(" << *pcn << ")" << endl;
#endif
    ...
    if (!pcnfd) {
        throw ExCLInternalError(
            "Could not downcast to a ClFDBinaryOneWayConstraint");
    }
...
pair<ClSymbolicWeight, FDNumber> ClFDSolver::ComputeBest(ClFDVariable *pcldv) {
    assert(pcldv);
    ...
    for (list<FDNumber>::const_iterator itVal = pcldv->PlfdnDomain()->begin();
         itVal != pcldv->PlfdnDomain()->end(); ++itVal) {
        ...
        for (ClConstraintSet::const_iterator itCn = setCns.begin();
             itCn != setCns.end(); ++itCn) {
            const ClConstraint *pcn = *itCn;
            ClSymbolicWeight e =
                ErrorForClvAtValSubjectToCn(pcldv, value, *pcn);
            error += e;
        }
...
ClSymbolicWeight
ClFDSolver::ErrorForClvAtValSubjectToCn(ClFDVariable *pcldv, FDNumber value,
                                        const ClConstraint &cn) {
    const ClFDBinaryOneWayConstraint *pcnFd =
        dynamic_cast<const ClFDBinaryOneWayConstraint *>(&cn);
    if (!pcnFd)
    ...
    case cnNEQ:
        if (value != rhs)
            ...
            e = 1; /* GJB:FIXME:: what makes sense here? */
        break;
    default:
        e = 0; /* quiet warning */
        assert(false);
...
ClFDSolver &ClFDSolver::ShowSolve() {
    topsort t;
    t.run(G);
    topsort::topsort_iterator it = t.top_order_begin();
    topsort::topsort_iterator end = t.top_order_end();
...
void ClFDSolver::ResetSetFlagsOnVariables() {
    for (ClVarToConstraintSetMap::iterator it = _mapClvToCns.begin();
         it != _mapClvToCns.end(); ++it) {
...
node ClFDSolver::GetVarNode(ClVariable v) {
