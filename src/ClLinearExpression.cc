...
template <class T> ClGenericLinearExpression<T>::~ClGenericLinearExpression() {}
...
// (private memfn)
template <class T>
ClGenericLinearExpression<T> &ClGenericLinearExpression<T>::MultiplyMe(T x) {
    _constant *= x;
...
template <class T>
ClGenericLinearExpression<T> ClGenericLinearExpression<T>::Times(
    const ClGenericLinearExpression<T> &expr) const {
    if (IsConstant()) {
...
template <class T>
ClGenericLinearExpression<T> ClGenericLinearExpression<T>::Plus(
    const ClGenericLinearExpression<T> &expr) const {
    ClGenericLinearExpression<T> result = *this;
...
template <class T>
ClGenericLinearExpression<T> ClGenericLinearExpression<T>::Minus(
    const ClGenericLinearExpression<T> &expr) const {
    ClGenericLinearExpression<T> result = *this;
    result.AddExpression(expr, -1.0);
    return result;
...
// (Note that this result must be linear.)
template <class T>
ClGenericLinearExpression<T> ClGenericLinearExpression<T>::Divide(
    const ClGenericLinearExpression<T> &expr) const {
    if (!expr.IsConstant()) {
        throw ExCLNonlinearExpression();
    }
...
template <class T>
ClGenericLinearExpression<T> ClGenericLinearExpression<T>::DivFrom(
    const ClGenericLinearExpression<T> &expr) const {
...
template <class T>
ClGenericLinearExpression<T> &ClGenericLinearExpression<T>::AddExpression(
    const ClGenericLinearExpression<T> &expr, Number n) {
...
template <class T>
ClGenericLinearExpression<T> &ClGenericLinearExpression<T>::AddExpression(
    const ClGenericLinearExpression<T> &expr, Number n, ClVariable subject,
    ClTableau &solver) {
    IncrementConstant(expr.Constant() * n);
...
template <class T>
ClGenericLinearExpression<T> &ClGenericLinearExpression<T>::AddVariable(
    ClVariable v, T c) { // body largely duplicated below
#ifdef CL_TRACE
...
template <class T>
ClGenericLinearExpression<T> &ClGenericLinearExpression<T>::AddVariable(
    ClVariable v, T c, ClVariable subject,
    ClTableau &solver) { // body largely duplicated above
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
...
template <class T>
ClVariable ClGenericLinearExpression<T>::AnyPivotableVariable() const {
    if (IsConstant()) {
        throw ExCLInternalError("(ExCLInternalError) No pivotable variables in "
                                "Constant expression");
    }
    typename ClVarToCoeffMap::const_iterator i = _terms.begin();
    for (; i != _terms.end(); ++i) {
...
template <class T>
void ClGenericLinearExpression<T>::SubstituteOut(
    ClVariable var, const ClGenericLinearExpression<T> &expr,
    ClVariable subject, ClTableau &solver) {
#ifdef CL_TRACE
    cerr << "* ClGenericLinearExpression::";
    Tracer TRACER(__FUNCTION__);
    cerr << "(" << var << ", " << expr << ", " << subject << ", " << solver
         << ")" << endl;
    cerr << "*this == " << *this << endl;
#endif
    ...
    for (; i != expr._terms.end(); ++i) {
        ...
        if (poc != _terms.end()) { // if oldCoeff is not nil
#ifdef CL_TRACE
            cerr << "Considering (*poc) == " << (*poc).second << "*"
                 << (*poc).first << endl;
#endif
        ...
        } else { // did not have that variable already (oldCoeff == nil)
#ifdef CL_TRACE
            cerr << "Adding (*i) == " << (*i).second << "*" << (*i).first
                 << endl;
#endif
...
template <class T>
void ClGenericLinearExpression<T>::ChangeSubject(ClVariable old_subject,
                                                 ClVariable new_subject) {
    _terms[old_subject] = NewSubject(new_subject);
}

inline double ReciprocalOf(double n) { return 1.0 / n; }
...
template <class T>
T ClGenericLinearExpression<T>::NewSubject(ClVariable subject) {
#ifdef CL_TRACE
    Tracer TRACER(__FUNCTION__);
    cerr << "(" << subject << ")" << endl;
#endif
...
template <class T> T ClGenericLinearExpression<T>::Evaluate() const {
    T answer = _constant;
    typename ClVarToCoeffMap::const_iterator i = _terms.begin();
