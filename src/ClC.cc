/* $Id: ClC.cc,v 1.23 2005/10/20 04:25:21 gjb Exp $
...
CL_VarMap CL_GetVarMap() { return varmap; }
...
void CL_VariableSetPv(CLV var, void *pv) { var->SetPv(pv); }

void *CL_VariablePv(CLV var) { return var->Pv(); }

const char *CL_VariableName(CLV var) { return var->Name().c_str(); }
...
void CL_SimplexSolverAddStrongStay(CL_SimplexSolver solver, CLV var,
                                   double weight) {
...
CLV CL_ClvLookupTrim(const char *szNameConst) {
    ...
    if (pchEnd)
        *pchEnd = ch; // restore the character
    return answer;
}
...
double CL_ClvValue(const CLV var) { return var->Value(); }

boolean CL_ClvIsNil(const CLV var) { return var->IsNil(); }
...
boolean CL_FIsSatisfied(ClConstraint *pcn) { return pcn->FIsSatisfied(); }
...
void CL_Solve(CL_Solver solver) { solver->Solve(); }

void CL_Resolve(CL_Solver solver) { solver->Resolve(); }

void CL_SimplexSolverSetEditedValue(CL_SimplexSolver solver, CLV var,
                                    double n) {
...
boolean CL_ClvIsFD(const CLV var) { return var->IsFDVariable(); }
