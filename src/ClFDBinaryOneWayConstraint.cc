...
ClFDBinaryOneWayConstraint::EnsurePreconditionsForCn(const ClConstraint &cn) {
    ...
    if (setRO.size() > 1)
        throw ExCLTooDifficultSpecial(
            "Only 0 or 1 read only variables are allowed");
    ...
    if (terms.size() == 2 && setRO.size() == 0)
        throw ExCLTooDifficultSpecial(
            "Both variables cannot be read-write, one must be read-only");
    if (terms.size() == 1 && setRO.size() == 1)
        throw ExCLTooDifficultSpecial("Single read-only variable in "
                                      "LinearConstraint -- must not be "
                                      "read-only.");
    ClVariable clv = (*terms.begin()).first;
    /* GJB:FIXME:: iterate over all the variables */
    if (!clv->IsFDVariable()) {
        throw ExCLTooDifficultSpecial(
            "FD constraint contains non-FD variables");
    }
}

bool ClFDBinaryOneWayConstraint::FCanConvertCn(const ClConstraint &cn) {
    try {
...
ClFDBinaryOneWayConstraint::ClFDBinaryOneWayConstraint(const ClConstraint &cn)
    ...
    for (ClVarToNumberMap::const_iterator it = terms.begin(); it != terms.end();
         ++it) {
    ...
    if (coeffRW == 0) {
        throw ExCLTooDifficultSpecial(
            "RW variable's coefficient must be non-zero");
    }
